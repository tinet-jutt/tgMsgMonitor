...
* **细粒度过滤规则**：支持对特定监听目标（群组、频道、机器人或私聊会话）设定匹配策略，支持包含词、排除词、**排除发送者（Username / 用户ID）**和**正则表达式**。
* **消息防抖冷却（Debounce）**：支持为规则设置防抖冷却时间（如 10 分钟）。触发一次推送后，在防抖冷却期内的后续消息不会再次触发发送，且每条尝试发送的匹配消息都会重新重置刷新防抖冷却期。
